...
class RaidHandler extends Handler {
    ...
    if (!args.length) {
      ...
      if (isInRaid) {
        if (args[0] == "join") {
          ...
          this.leaveRaid(message, raidEvent);
        }
      }else{
  ...
  async joinRaid(message, args, raidEvent){
    ...
    if (args.length < 2) {
      ...
      //(args[1].toUpperCase() != "dps" || args[1].toUpperCase() != "tank" || args[1].toUpperCase() != "heal")
      return message.channel.send(
        this.embed("error", "🏰 Raid Join 🏰", {
  ...
  async leaveRaid(message, raidEvent){
    ...
    if(isPartOfRaid){
      for (var i = 0; i < raidEvent.members.length; i++){
        if (message.author.id == raidEvent.members[i].id){
   ...
   regenerateRaidMessage(message, raidEvent){
      let dps = [];
      let tank = [];
      let heal = [];
